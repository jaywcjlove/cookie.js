...
> 开启浏览器的调试工具，调试看浏览器数据是否设置成功！

<script type="text/javascript" src="https://rawgit.com/jaywcjlove/cookie.js/master/cookie.js"></script>

<a href="javascript:getcookie()">获取所有cookie</a>，
<a href="javascript:cleancookie()">清空cookie</a>
<input type="text" id="cookieval" value=""> 
<a href="javascript:setcookie()">设置cookie</a>
<div id="cookieinfo"></div>

<script type="text/javascript">
    var val = document.getElementById("cookieval"),
        inof = document.getElementById('cookieinfo');

    val.value = '{"name1":"value1"}';
    function getcookie(){
        inof.innerHTML = JSON.stringify(cookie.all());
    }
    function setcookie(){
        console.log('设置的键值对：',val.value)
        cookie(JSON.parse(val.value))
        inof.innerHTML = '设置成功！' +  val.value;
    }
